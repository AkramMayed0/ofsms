...
CREATE TABLE IF NOT EXISTS disbursement_lists (
  ...
  approved_by_supervisor UUID REFERENCES users(id) ON DELETE
  SET
    NULL,
    approved_by_finance UUID REFERENCES users(id) ON DELETE
  SET
    NULL,
    approved_by_gm UUID REFERENCES users(id) ON DELETE
  SET
    NULL,
...
CREATE TRIGGER trg_disbursement_lists_updated_at BEFORE
UPDATE
  ON disbursement_lists FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE IF NOT EXISTS biometric_receipts (
  ...
  item_id UUID NOT NULL UNIQUE REFERENCES disbursement_items(id) ON DELETE CASCADE,
