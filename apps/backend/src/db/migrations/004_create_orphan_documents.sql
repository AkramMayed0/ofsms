...
CREATE TYPE doc_type AS ENUM (
  'death_cert',    -- شهادة وفاة الأب   (required)
  'birth_cert',    -- شهادة الميلاد     (required)
  'guardian_id',   -- هوية الوصي        (optional)
...
CREATE TABLE IF NOT EXISTS orphan_documents (
  ...
  orphan_id     UUID        NOT NULL
                  REFERENCES orphans(id)
                  ON DELETE CASCADE,   -- delete docs when orphan is deleted
...
CREATE INDEX IF NOT EXISTS idx_orphan_documents_orphan_id
  ON orphan_documents(orphan_id);

-- Secondary: filter by document type within an orphan
CREATE INDEX IF NOT EXISTS idx_orphan_documents_type
  ON orphan_documents(orphan_id, doc_type);

-- ── Auto-update updated_at ────────────────────────────────────
CREATE TRIGGER trg_orphan_documents_updated_at
  BEFORE UPDATE ON orphan_documents
