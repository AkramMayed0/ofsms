...
const getSupervisorDashboard = async () => {
  ...
  if (now.getDate() > disbursementDay) nextDisbursement.setMonth(nextDisbursement.getMonth() + 1);
...
const getFinanceDashboard = async () => {
  // Disbursement lists awaiting finance authorization
  const { rows: awaitingAuth } = await query(`
    SELECT
  ...
  // Recent authorization history (last 10)
  const { rows: recentHistory } = await query(`
    SELECT
      dl.id, dl.month, dl.year, dl.status,
