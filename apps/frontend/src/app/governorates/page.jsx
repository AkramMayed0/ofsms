 ...
 *
 * Left panel  → list of all governorates with orphan count bars
 * Right panel → click a governorate to see its orphans in a table
 *
 * APIs:
 *   GET /api/dashboard/gm                  → orphans_per_governorate[]
 *   GET /api/governorates/:id/orphans      → orphans for selected governorate
 *   GET /api/governorates                  → full governorate list (with IDs)
 */
...
export default function GovernoratesPage() {
  const [govStats, setGovStats]       = useState([]); // { governorate_ar, count, id }
  const [allGovs, setAllGovs]         = useState([]); // full list with IDs
  ...
  // Load governorate stats + full gov list
  useEffect(() => {
    Promise.all([
      ...
      .then(([dashRes, govsRes]) => {
        ...
        // Merge: attach IDs from the full list to stats
  ...
  // Load orphans for selected governorate
  ...
  // Stats for selected governorate
    ...
    <AppShell>
      <div dir="rtl" style={{ fontFamily: "'Cairo','Tajawal',sans-serif", height: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <style>{`
          ...
          .gv-filter:hover   { border-color: #1B5E8C !important; color: #1B5E8C !important; }
        `}</style>
                ...
                govStats.map(gov => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '.65rem .85rem', borderRadius: '.75rem', marginBottom: '.3rem',
                      border: '1.5px solid transparent', cursor: gov.count > 0 ? 'pointer' : 'default',
                      opacity: gov.count === 0 ? 0.45 : 1, transition: 'all .15s',
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                        {selected?.id === gov.id && (
                          <span style={{ color: '#1B5E8C', fontWeight: 800, fontSize: '.8rem' }}>▶</span>
                        )}
                        <span style={{ fontSize: '.85rem', fontWeight: 600, color: '#1f2937' }}>{gov.name}</span>
                      </div>
                      <span style={{
                        fontSize: '.72rem', fontWeight: 800, minWidth: '1.6rem', textAlign: 'center',
                        padding: '.15rem .5rem', borderRadius: '2rem',
                        background: gov.count > 0 ? '#1B5E8C' : '#e5e7eb',
                        color: gov.count > 0 ? '#fff' : '#9ca3af',
                      }}>
                        {gov.count}
                      </span>
                    </div>
                    ...
                    <div style={{ height: 5, background: '#f0f4f8', borderRadius: '999px', overflow: 'hidden' }}>
                      <div style={{
                        ...
                        width: `${(gov.count / maxCount) * 100}%`,
                        background: selected?.id === gov.id
                          ? 'linear-gradient(90deg,#1B5E8C,#2E7EB8)'
                          : 'linear-gradient(90deg,#93c5fd,#60a5fa)',
                        animation: 'gv-bar .6s ease both',
                      }} />
                    </div>
                  </div>
                ))
              ...
              <>
                ...
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #f0f4f8', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', marginBottom: '.75rem' }}>
                    <div>
                      <h2 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0d3d5c', margin: '0 0 .15rem' }}>
                        📍 {selected.name}
                      </h2>
                        ...
                        {Object.entries(statusCounts).map(([status, count]) => {
                          ...
                          return (
                            <span key={status} style={{
                              fontSize: '.68rem', fontWeight: 700, padding: '.2rem .6rem',
                              borderRadius: '2rem', background: cfg.bg, color: cfg.color,
                            }}>
                              {cfg.label}: {count}
                            </span>
                          );
                      ...
                      <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
                        ...
                        <input
                          ...
                          style={{
                            ...
                            padding: '.55rem .85rem .55rem 2rem', paddingRight: '2.2rem',
                            fontSize: '.82rem', fontFamily: 'Cairo,sans-serif',
                      ...
                      <div style={{ display: 'flex', gap: '.35rem', flexWrap: 'wrap' }}>
                        {[{ key: 'all', label: 'الكل' }, ...Object.entries(STATUS).map(([k, v]) => ({ key: k, label: v.label }))].map(({ key, label }) => (
                          <button
                            ...
                            style={{
                              padding: '.3rem .7rem',
                              border: `1.5px solid ${statusFilter === key ? '#1B5E8C' : '#e5eaf0'}`,
                              borderRadius: '2rem', fontSize: '.72rem', fontWeight: 600,
                              color: statusFilter === key ? '#fff' : '#6b7280',
                              background: statusFilter === key ? '#1B5E8C' : '#fff',
                  ...
                  {loadingRight ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '.5rem', padding: '3rem 2rem', color: '#9ca3af', textAlign: 'center' }}>
                      ...
                      <p style={{ fontSize: '.8rem', margin: 0 }}>
                        {orphans.length === 0 ? 'لم يُسجَّل أي يتيم في هذه المحافظة بعد' : 'جرّب تغيير معايير البحث أو الفلترة'}
                      </p>
                    </div>
                  ) : (
                        ...
                        <thead>
                          <tr style={{ background: '#f8fafc', position: 'sticky', top: 0, zIndex: 1 }}>
                            {['#', 'الاسم', 'العمر', 'الحالة', 'الكافل', 'المندوب', 'تاريخ التسجيل'].map(h => (
                              <th key={h} style={{ padding: '.75rem .9rem', textAlign: 'right', fontSize: '.72rem', fontWeight: 700, color: '#6b7a8d', whiteSpace: 'nowrap', borderBottom: '2px solid #e5eaf0' }}>
                                {h}
                              </th>
                            ))}
                          </tr>
                          ...
                          {filteredOrphans.map((o, idx) => {
                            ...
                            return (
                              <tr key={o.id} className="gv-orphan-row">
                                ...
                                <td style={{ padding: '.75rem .9rem', borderBottom: '1px solid #f8fafc' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                                    ...
                                    <div>
                                      ...
                                      {o.is_gifted && (
                                        <span style={{ fontSize: '.63rem', fontWeight: 700, color: '#f59e0b' }}>🌟 موهوب</span>
                                      )}
                                    </div>
                                  </div>
                                </td>
                                <td style={{ padding: '.75rem .9rem', borderBottom: '1px solid #f8fafc', color: '#6b7a8d', whiteSpace: 'nowrap' }}>
                                  {calcAge(o.date_of_birth)}
                                </td>
                                <td style={{ padding: '.75rem .9rem', borderBottom: '1px solid #f8fafc' }}>
                                  <span style={{ fontSize: '.7rem', fontWeight: 700, padding: '.2rem .6rem', borderRadius: '2rem', background: cfg.bg, color: cfg.color, whiteSpace: 'nowrap' }}>
                                    {cfg.label}
                                  </span>
                                </td>
                                ...
                                <td style={{ padding: '.75rem .9rem', borderBottom: '1px solid #f8fafc', color: '#6b7a8d', fontSize: '.8rem' }}>
                                  {o.agent_name || '—'}
                                </td>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '.4rem', padding: '.25rem' }}>
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} style={{
          height: 52, borderRadius: '.75rem',
          background: 'linear-gradient(90deg,#f3f4f6 25%,#e5e7eb 50%,#f3f4f6 75%)',
          backgroundSize: '200% 100%', animation: `gv-spin ${1.2 + i * 0.05}s infinite`,
        }} />
      ))}
    </div>
  );
  ...
  return (
    <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} style={{
          height: 44, borderRadius: '.625rem',
          background: 'linear-gradient(90deg,#f3f4f6 25%,#e5e7eb 50%,#f3f4f6 75%)',
          backgroundSize: '200% 100%', animation: 'gv-spin 1.4s infinite',
        }} />
      ))}
    </div>
  );
