  ...
  return (
    <div className="table-section">
      <div className="table-meta">
        ...
        <span className="table-total">الإجمالي: <strong>{totalIncluded.toLocaleString('ar-YE')} ريال</strong></span>
      </div>
...
export default function DisbursementDetailPage() {
  ...
  return (
    <AppShell>
      <div className="page" dir="rtl">
        ...
        <div className="page-top">
          ...
          <div className="page-top-actions">
            ...
            <ExportButtons
              ...
              filename={`كشف-صرف-${monthName}-${list.year}`}
              variant="menu"
            />
          </div>
        </div>
        ...
        <div className="timeline-card">
          ...
          <div className="timeline">
            {[
              { label: 'إنشاء الكشف',      done: true,                                                             at: list.created_at },
              { label: 'اعتماد المشرف',    done: ['supervisor_approved','finance_approved','released'].includes(list.status), at: list.supervisor_approved_at },
              { label: 'مصادقة المالي',    done: ['finance_approved','released'].includes(list.status),            at: list.finance_approved_at },
              { label: 'الإصدار النهائي',  done: list.status === 'released',                                       at: list.gm_approved_at },
            ].map((step, i) => (
        ...
        {isReleased  && <div className="released-banner">🚀 تم إصدار كشف الصرف بنجاح وأُشعر المناديب بتوزيع المبالغ.</div>}

        {/* Action buttons */}
        {(canSupApprove || canFinApprove || canGmRelease) && (
          <div className="actions-bar">
            <span className="actions-label">الإجراءات المتاحة:</span>
            {canSupApprove && <button className="btn-approve" onClick={() => setShowSupApprove(true)}>✅ اعتماد المشرف</button>}
            {canSupReject  && <button className="btn-reject"  onClick={() => setShowSupReject(true)}>❌ رفض</button>}
            {canFinApprove && <button className="btn-approve" onClick={() => setShowFinApprove(true)}>💜 مصادقة المالي</button>}
            {canFinReject  && <button className="btn-reject"  onClick={() => setShowFinReject(true)}>❌ رفض</button>}
            {canGmRelease  && <button className="btn-release" onClick={() => setShowRelease(true)}>🚀 إصدار أمر الصرف</button>}
          </div>
        )}
      ...
      {/* Modals */}
      <ConfirmModal open={showSupApprove} title="اعتماد كشف الصرف"
        ...
        onCancel={() => setShowSupApprove(false)} loading={actionLoading} />

      <ConfirmModal open={showFinApprove} title="مصادقة القسم المالي"
        ...
        onCancel={() => setShowFinApprove(false)} loading={actionLoading} />

      <ConfirmModal open={showRelease} title="إصدار أمر الصرف النهائي"
        message={`هل تريد إصدار كشف صرف شهر ${monthName} ${list.year} نهائياً؟ قيمة إجمالية ${totalAmount.toLocaleString('ar-YE')} ريال.`}
        onConfirm={() => { setShowRelease(false); doAction('gm-release', {}, `تم إصدار كشف الصرف بنجاح 🚀`); }}
        onCancel={() => setShowRelease(false)} loading={actionLoading} />

      <RejectModal open={showSupReject}
        onConfirm={(notes) => { setShowSupReject(false); doAction('supervisor-reject', { notes }, 'تم رفض الكشف وإشعار المُنشئ'); }}
        onCancel={() => setShowSupReject(false)} loading={actionLoading} />

      <RejectModal open={showFinReject}
        onConfirm={(notes) => { setShowFinReject(false); doAction('finance-reject', { notes }, 'تم رفض الكشف وإعادته للمشرف'); }}
        onCancel={() => setShowFinReject(false)} loading={actionLoading} />
      ...
      <style jsx>{`
        ...
        .page-top { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:.75rem; }
        .page-heading { display:flex; align-items:center; gap:1rem; }
        .page-top-actions { display:flex; align-items:center; gap:.75rem; flex-wrap:wrap; }
        ...
        .timeline { display:flex; align-items:flex-start; overflow-x:auto; padding-bottom:.25rem; }
        .tl-item { display:flex; align-items:flex-start; flex:1; min-width:120px; }
        ...
        .btn-approve { background:#1B5E8C; color:#fff; border:none; border-radius:.625rem; padding:.65rem 1.4rem; font-family:'Cairo',sans-serif; font-size:.88rem; font-weight:700; cursor:pointer; }
        ...
        .btn-release { background:linear-gradient(135deg,#1B5E8C,#134569); color:#fff; border:none; border-radius:.75rem; padding:.75rem 2rem; font-family:'Cairo',sans-serif; font-size:.95rem; font-weight:800; cursor:pointer; box-shadow:0 2px 12px rgba(27,94,140,.3); transition:all .15s; }
