 ...
 *
 * GM sees the full approval chain and a "إصدار أمر الصرف" release button.
 * Supervisor sees approve/reject for draft lists.
 * Finance sees approve/reject for supervisor-approved lists.
 */
...
export default function DisbursementDetailPage() {
  ...
  console.log('PARAMS ID:', id);  // add this at the top of the component
const fetchData = () => {
  setLoading(true);
  setError('');
  setData(null);
  api.get(`/disbursements/${id}`)
    .then((axiosResponse) => {
      ...
      console.log('DATA STRING:', JSON.stringify(res).slice(0, 500));
      
if (res && res.id) {
  // backend returns flat: { id, month, status, ..., items: [] }
  // reshape to what the component expects: { list, items }
  ...
  return (
    <AppShell>
      <div className="page" dir="rtl">
        ...
        {actionError && (
          <div className="err-banner">⚠ {actionError}</div>
        )}
        ...
        {(canSupApprove || canFinApprove || canGmRelease) && (
          <div className="actions-bar">
            ...
            {canSupReject && (
              <button className="btn-reject" onClick={() => setShowSupReject(true)}>
                ❌ رفض
              </button>
            )}
            {canFinApprove && (
            ...
            {canFinReject && (
              <button className="btn-reject" onClick={() => setShowFinReject(true)}>
                ❌ رفض
              </button>
            )}
            {canGmRelease && (
      ...
      <ConfirmModal
        ...
        message={`هل تريد إصدار كشف صرف شهر ${monthName} ${list.year} نهائياً؟ سيتلقى المناديب إشعاراً بالتوزيع وقيمة إجمالية ${totalAmount.toLocaleString('ar-YE')} ريال.`}
